...
\abstract{Windowed machinery benchmarks can overstate diagnostic transfer when correlated windows from one acquisition recording cross data partitions. We establish a recording-grouped benchmark on the MCC5-THU Motor bearing subset and examine transfer direction, partition sensitivity, feature membership, and residual session-proxy structure. The study includes 84 recordings, 15,036 overlapping windows, and 254 numeric features; neural protocols use frozen recording roles and training-only z-score statistics. Across ten matched splits, random assignment of overlapping windows yields 0.9976 $\pm$ 0.0009 macro-F1 while sharing all 84 source recordings between training and test, whereas recording-grouped assignment yields 0.9777 $\pm$ 0.0356 with zero source overlap. Leave-one-speed-out performance is directional: random-forest macro-F1 remains near 0.99 when 1000 or 2000 rpm is withheld but falls to 0.6921 for upward extrapolation from 1000/2000 to the nominal 3000-rpm domain. Recording-level aggregation preserves this boundary at 0.7020 macro-F1 over 28 test recordings. A same-date, three-fault control remains perfectly separable across ten grouped splits, but training-only date metadata alone yields 0.7028--0.7723 macro-F1 across the four protocols, and signals from outer-race recordings predict acquisition date with 0.8635 mean accuracy. These complementary controls support genuine fault discrimination while showing that recording grouping does not remove all session-proxy information. The contribution is a direction-aware MCC5 benchmark, not evidence of specimen-, session-, or machine-independent generalization.}
...
The contributions are threefold. First, a frozen recording-grouped MCC5 benchmark is reported with explicit transfer directions and a matched comparison against random overlapping-window assignment. Second, repeated grouped partitions, a complete classical-model recording-level matrix, and class-stratified recording bootstrap distinguish partition, optimization, and finite-test-recording variability. Third, exact feature and input-schema audits are combined with raw-file identity, same-date discrimination, metadata-only, and within-class date-predictability controls. The resulting paper is a benchmark and sensitivity study, not a claim of universal model superiority.
...
Three complementary acquisition-date controls use dates parsed from public recording identifiers. First, the 47 recordings acquired on 250707 provide a same-date, recording-grouped three-fault task; healthy recordings are unavailable on that date. Second, date-only, operating-context-only, and combined metadata classifiers are fitted on the training recordings of each frozen protocol and evaluated on its test recordings. Third, acquisition date is predicted within the 25 outer-race recordings using ten repeated source-grouped folds. This last task holds fault location fixed and tests whether the measured descriptors retain date-related session signatures; its rarest date contains only two recordings, so accuracy, macro-F1, and worst-class recall are reported together.
...
The recordings span five acquisition dates, and class composition is uneven across them. An in-sample date-majority mapping agrees with 61 of 84 labels (0.7262), with Cramer's V of 0.7714 and normalized mutual information of 0.5989. The controls in Table~\ref{tab:session-controls} qualify this association. Both classical models separate the three same-date fault classes perfectly across ten grouped splits, indicating that fault information remains when date is fixed. Conversely, training-only date metadata alone reaches 0.7028--0.7723 macro-F1 across the four frozen tests, and an XGBoost model predicts acquisition date within outer-race recordings with 0.8635 $\pm$ 0.0775 accuracy but only 0.5995 $\pm$ 0.0547 macro-F1 and zero worst-date recall. Thus, the evidence supports fault discrimination and residual date-linked signal structure simultaneously; it does not establish session-independent diagnosis.

\begin{table}[H]
\centering
\caption{Matched partition and acquisition-date controls. Split variability uses ten split seeds with model seed 42. The outer-race date task uses ten repeated source-grouped folds.}
...
Outer-race acquisition-date prediction & Recording & 0 & Accuracy / macro-F1 & 0.8635 / 0.5995 \\
\bottomrule
\end{tabular}}
\end{table}
...
Raw-file hashing excludes byte-identical recordings, including within the repeated-timestamp group, but it does not resolve shared specimen or session identity. The acquisition-date controls give a mixed but informative result. Perfect same-date three-fault separation shows that date is not necessary for those fault decisions; the date-only training baseline and within-outer-race date-prediction task show that date-linked nuisance structure is nevertheless available. Recording grouping removes direct window sharing, not this broader session-proxy dependence.
...
\section{Limitations}
The study uses one controlled rig and one bearing subset. Physical specimen identity is not available, severity levels are pooled, and no external machine or independent bearing set is harmonized sufficiently for confirmatory validation. Five acquisition dates are unevenly associated with class (Cramer's V 0.7714). The same-date task omits healthy recordings and covers only one date, while outer-race date prediction has only two recordings in its rarest date; these controls diagnose but cannot remove session confounding. The 26/28-field auxiliary configurations are post-hoc. Neural seed counts are modest, and classical directional experiments use fixed hyperparameters rather than nested optimization. The strict full branch still uses a compact architecture; its 3000-rpm failure does not imply that all fusion strategies will fail.
...
\section{Conclusions}
This study provides a recording-grouped MCC5 bearing benchmark with explicit operating-shift direction and train-only standardization. A matched control shows that random window assignment gives uniformly optimistic results while allowing every recording to cross roles; recording grouping lowers the mean modestly but exposes much larger variation among held-out recordings. Classical models remain strong on source-recording, cross-mode, and cross-load protocols, while upward extrapolation from 1000/2000 to the nominal 3000-rpm domain is the dominant boundary across classical and neural evidence.

The input audit also clarifies the fusion interpretation. Strict-254 scalar-branch fusion is strong on easier protocols but collapses at 3000 rpm, while RPM/load alone adds little to the vibration-current branch. Same-date fault discrimination remains strong, yet metadata-only and within-class date controls reveal residual session-proxy information. Post-hoc auxiliary-bundle results are retained only as transparent Supplementary evidence and do not support model superiority. The contribution is therefore a reproducible directional benchmark and sensitivity audit, not proof of specimen independence, universal order invariance, session-independent diagnosis, or confirmatory auxiliary-fusion superiority.
