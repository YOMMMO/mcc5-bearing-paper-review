\documentclass[machines,article,submit,oneauthor]{Definitions/mdpi}
\renewcommand\linenumberfont{\normalfont\tiny\fontfamily{phv}\selectfont}
...
\Title{Recording-Grouped Bearing Fault Diagnosis under Directional Operating-Condition Shifts on the MCC5 Electric-Drive Benchmark}
\Author{Yu Li $^{1}$}
\AuthorNames{Yu Li}
\address{%
$^{1}$ \quad Shanxi Engineering Vocational College, China}

\abstract{Windowed machinery benchmarks can overstate diagnostic transfer when correlated windows from one acquisition recording cross data partitions. We establish a recording-grouped benchmark on the MCC5-THU Motor bearing subset and examine transfer direction, grouped-partition variability, feature membership, and input semantics. The study includes 84 recordings, 15,036 overlapping windows, and 254 numeric features; neural protocols use frozen recording roles and training-only z-score statistics. XGBoost achieves 0.9964 macro-F1 on the predefined source-recording split and 0.9777 $\pm$ 0.0356 across ten grouped partitions. Random forest achieves 0.9790 for speed-circulation-to-torque-circulation transfer and 0.9891 for 20-to-40 Nm transfer. Leave-one-speed-out performance is strongly directional: macro-F1 remains near 0.99 when 1000 or 2000 rpm is withheld but falls to 0.6921 when extrapolating from 1000/2000 to 3000 rpm. Under the same high-speed holdout, the predefined 8,192-sample CNN and strict-254 scalar-branch fusion also deteriorate, while retaining the full one-second sequence does not remove the boundary. A separately labeled post-hoc input audit indicates that RPM/load alone does not account for the higher same-holdout performance associated with the auxiliary-26 bundle, which comprises two modality-presence indicators and 24 torque/key-phase waveform summaries. The contribution is a direction-aware, recording-grouped MCC5 benchmark, not evidence of specimen-independent or universal speed generalization.}
\keyword{bearing fault diagnosis; electric-drive condition monitoring; recording-grouped evaluation; directional operating-condition transfer; MCC5}
...
The contributions are threefold. First, a frozen recording-grouped MCC5 benchmark is reported with explicit transfer directions and zero recording overlap across neural training, validation, and test roles. Second, repeated grouped partitions, recording-level aggregation, and class-stratified recording bootstrap are used to separate optimization variability from data uncertainty. Third, exact input audits distinguish two nominal context variables; an auxiliary-26 setting containing two modality-presence indicators and 24 waveform summaries (12 torque and 12 key-phase); a strict 254-feature engineered scalar branch; and a legacy 262-field implementation. The resulting paper is a benchmark and evidence-audit study, not a claim of universal model superiority.
...
\begin{figure}[H]
\centering\includegraphics[width=0.96\textwidth]{figures/figure_1_workflow.pdf}
...
Speed-circulation \ensuremath{\rightarrow} torque-circulation & 34 & 8 & 42 & 0 / 0 / 0 & No \\
20 \ensuremath{\rightarrow} 40 Nm & 34 & 8 & 42 & 0 / 0 / 0 & No \\
1000/2000 \ensuremath{\rightarrow} 3000 rpm & 45 & 11 & 28 & 0 / 0 / 0 & No \\
\bottomrule
\end{tabular}}
...
The branch network has vibration and current encoders, an optional scalar multilayer perceptron, sigmoid gating, and a classifier head. Six input definitions are distinguished: no scalar inputs; exactly RPM and load; auxiliary-26 with two modality-presence indicators and 24 waveform summaries (12 torque and 12 key-phase); auxiliary-context-28, which adds RPM/load; a vibration-current network with a strict 254-feature engineered scalar branch (strict-254 scalar-branch fusion); and a legacy 262-field branch that additionally includes four window-position and four modality-availability fields. The auxiliary-26 and auxiliary-context-28 settings were identified after inspecting the 3000-rpm holdout and are therefore labeled post-hoc exploratory.

\subsection{Metrics and uncertainty}
Macro-F1 is primary, worst-class recall identifies class collapse, and accuracy is supporting. Three optimization seeds are used generally and five for the focused 3000-rpm input audit. Seed standard deviation describes training variability only. For recording-level evaluation, window-level class probabilities are averaged within each test recording before assigning the recording label. Class-stratified bootstrap resamples complete recordings 2,000 times; overlapping windows are never sampled as independent units.

\section{Results}
\subsection{Directional classical benchmark}
Classical feature models remain strong on source-recording, cross-mode, and cross-load protocols. Across ten grouped source-recording partitions, XGBoost reaches 0.9777 $\pm$ 0.0356 macro-F1, with a range of 0.9092--0.9992. Direction changes the conclusion most strongly for speed. Holding out 1000 or 2000 rpm remains near 0.99, whereas holding out 3000 rpm reduces random-forest macro-F1 to 0.6921 and worst-class recall to 0.1099.
...
\midrule
Speed-circulation \ensuremath{\rightarrow} torque-circulation & Random forest & 0.9790 & 0.9167 & 0.9761 \\
Torque-circulation \ensuremath{\rightarrow} speed-circulation & Random forest & 0.9379 & 0.8822 & 0.9307 \\
20 \ensuremath{\rightarrow} 40 Nm & Random forest & 0.9891 & 0.9795 & 0.9891 \\
40 \ensuremath{\rightarrow} 20 Nm & Random forest & 0.9564 & 0.8457 & 0.9507 \\
...
\begin{figure}[H]
\centering\includegraphics[width=0.98\textwidth]{figures/figure_2_directional_robustness.pdf}
...
\begin{figure}[H]
\centering\includegraphics[width=0.98\textwidth]{figures/figure_3_partition_sensitivity.pdf}
...
\begin{figure}[H]
\centering\includegraphics[width=0.98\textwidth]{figures/figure_4_exact_feature_blocks.pdf}
...
\begin{tabular}{lcccc}
\toprule
Protocol & Model & Macro-F1 mean \ensuremath{\pm} SD & Worst recall mean \ensuremath{\pm} SD & Seeds \\
\midrule
Source-recording & CNN & 0.8535 \ensuremath{\pm} 0.0286 & 0.5768 \ensuremath{\pm} 0.0631 & 3 \\
Source-recording & TCN & 0.8759 \ensuremath{\pm} 0.0263 & 0.5964 \ensuremath{\pm} 0.0840 & 3 \\
Source-recording & Transformer & 0.9531 \ensuremath{\pm} 0.0289 & 0.8762 \ensuremath{\pm} 0.0804 & 3 \\
Cross-mode & CNN & 0.9501 \ensuremath{\pm} 0.0076 & 0.8703 \ensuremath{\pm} 0.0506 & 3 \\
Cross-mode & TCN & 0.9432 \ensuremath{\pm} 0.0115 & 0.8749 \ensuremath{\pm} 0.0367 & 3 \\
Cross-mode & Transformer & 0.9356 \ensuremath{\pm} 0.0092 & 0.8915 \ensuremath{\pm} 0.0320 & 3 \\
Cross-load & CNN & 0.8750 \ensuremath{\pm} 0.0256 & 0.7337 \ensuremath{\pm} 0.0839 & 3 \\
Cross-load & TCN & 0.7302 \ensuremath{\pm} 0.0989 & 0.3726 \ensuremath{\pm} 0.1638 & 3 \\
Cross-load & Transformer & 0.9076 \ensuremath{\pm} 0.0602 & 0.7981 \ensuremath{\pm} 0.1276 & 3 \\
1000/2000 \ensuremath{\rightarrow} 3000 rpm & CNN & 0.6644 \ensuremath{\pm} 0.1523 & 0.4270 \ensuremath{\pm} 0.2296 & 3 \\
1000/2000 \ensuremath{\rightarrow} 3000 rpm & TCN & 0.5011 \ensuremath{\pm} 0.0465 & 0.2381 \ensuremath{\pm} 0.1018 & 3 \\
1000/2000 \ensuremath{\rightarrow} 3000 rpm & Transformer & 0.4703 \ensuremath{\pm} 0.0810 & 0.1932 \ensuremath{\pm} 0.0463 & 3 \\
\bottomrule
\end{tabular}}
...
\begin{figure}[H]
\centering\includegraphics[width=0.92\textwidth]{figures/figure_5_corrected_raw_models.pdf}
...
The focused input audit separates signal roles. No-scalar and RPM/load-only branches reach 0.7081 and 0.7124 mean macro-F1, respectively. Auxiliary-26 is shown once in Table~\ref{tab:fusion} to disclose the higher same-holdout result, but its complete four-protocol, per-seed, recording-level, and bootstrap evidence is confined to the Supplementary Material. Because that configuration was selected after viewing the same holdout, it remains hypothesis-generating rather than confirmatory.


\begin{table}[H]
\centering
\caption{Focused input-definition audit for the 1000/2000-to-3000-rpm holdout. Values are means and standard deviations across five optimization seeds. The starred auxiliary-26 configuration was identified after inspection of the same holdout and is post-hoc exploratory.}
\label{tab:fusion}
\small
\resizebox{\textwidth}{!}{%
\begin{tabular}{lcccc}
\toprule
Configuration & Scalar inputs & Macro-F1 mean \ensuremath{\pm} SD & Worst recall mean \ensuremath{\pm} SD & Evidence status \\
\midrule
Vibration-current; no scalars & 0 & 0.7081 \ensuremath{\pm} 0.0986 & 0.4324 \ensuremath{\pm} 0.1507 & Matched protocol \\
Vibration-current + RPM/load & 2 & 0.7124 \ensuremath{\pm} 0.0529 & 0.4272 \ensuremath{\pm} 0.0964 & Matched protocol \\
Strict-254 scalar-branch fusion & 254 & 0.4001 \ensuremath{\pm} 0.0813 & 0.0257 \ensuremath{\pm} 0.0045 & Matched protocol \\
\midrule
Vibration-current + auxiliary-26* & 26 & 0.9162 \ensuremath{\pm} 0.0245 & 0.8179 \ensuremath{\pm} 0.0593 & Post-hoc exploratory \\
\bottomrule
\end{tabular}}
\vspace{0.3em}\parbox{\textwidth}{\footnotesize * Post-hoc configuration identified after inspection of the same 3000-rpm holdout.}
\end{table}


\subsection{Recording-level evidence and input duration}
Recording aggregation exposes class-level fragility that window counts can obscure. Strict-254 scalar-branch fusion has zero recording-level worst-class recall on every seed at 3000 rpm. Detailed auxiliary-26 recording results remain in the Supplementary Material because that configuration is post-hoc. All values in Table~\ref{tab:source} are based on 28 test recordings rather than thousands of correlated windows.
...
\begin{tabular}{lcccc}
\toprule
Model/configuration & Test recordings & Seeds & Recording macro-F1 mean \ensuremath{\pm} SD & Recording worst recall mean \ensuremath{\pm} SD \\
\midrule
Raw CNN & 28 & 3 & 0.7233 \ensuremath{\pm} 0.2492 & 0.4722 \ensuremath{\pm} 0.4111 \\
No-scalar branch & 28 & 5 & 0.7122 \ensuremath{\pm} 0.1238 & 0.4000 \ensuremath{\pm} 0.2054 \\
RPM/load branch & 28 & 5 & 0.7306 \ensuremath{\pm} 0.0917 & 0.4000 \ensuremath{\pm} 0.1369 \\
Strict-254 scalar-branch fusion & 28 & 5 & 0.3660 \ensuremath{\pm} 0.1254 & 0.0000 \ensuremath{\pm} 0.0000 \\
\bottomrule
\end{tabular}}
...
\begin{tabular}{lccc}
\toprule
Retained samples & Macro-F1 mean \ensuremath{\pm} SD & Worst recall mean \ensuremath{\pm} SD & Batched inference time (ms/window) \\
\midrule
8192 & 0.6644 \ensuremath{\pm} 0.1523 & 0.4270 \ensuremath{\pm} 0.2296 & 0.0432 \\
12800 & 0.7187 \ensuremath{\pm} 0.0578 & 0.4986 \ensuremath{\pm} 0.1657 & 0.0661 \\
\bottomrule
\end{tabular}}
...
The input audit supports two distinctions. First, strict-254 scalar-branch fusion performs strongly on three protocols with train-only z-score scaling but remains weak on the 3000-rpm holdout; this does not support a general claim that multisource fusion is defective. Second, RPM/load alone does not explain the high post-hoc auxiliary result. The auxiliary-26 input contains two presence indicators and 24 torque/key-phase waveform summaries. These measured signals are plausibly informative, but same-holdout discovery creates selection bias. Independent locked confirmation or nested condition-level selection is needed before presenting that branch as a method contribution.
...
\section{Limitations}
The study uses one controlled rig and one bearing subset. Physical specimen identity is not available, severity levels are pooled, and no external machine or independent bearing set is harmonized sufficiently for confirmatory validation. The 26/28-field auxiliary configurations are post-hoc. Neural seed counts are modest, and classical directional experiments use fixed hyperparameters rather than nested optimization. The strict full branch still uses a compact architecture; its 3000-rpm failure does not imply that all fusion strategies will fail.
...
The input audit also clarifies the fusion interpretation. Strict-254 scalar-branch fusion is strong on easier protocols but collapses at 3000 rpm. RPM/load alone adds little, whereas auxiliary presence indicators and torque/key-phase waveform summaries are associated with substantially higher performance on the same inspected holdout. Because this configuration was identified post hoc, the result remains hypothesis-generating rather than confirmatory. The contribution is therefore a reproducible directional benchmark and input-definition audit, not proof of specimen independence, universal order invariance, or confirmatory auxiliary-fusion superiority.

\supplementary{The supporting information includes the complete fusion-input protocol table, per-seed focused results, recording-level evidence, severity and per-recording probability audits, recording-cluster bootstrap intervals, model architectures and fixed hyperparameters, and a versioned software supplement containing machine-readable evidence and reproduction materials.}

\authorcontributions{Conceptualization, Y.L.; methodology, Y.L.; software, Y.L.; validation, Y.L.; formal analysis, Y.L.; investigation, Y.L.; data curation, Y.L.; visualization, Y.L.; writing---original draft preparation, Y.L.; writing---review and editing, Y.L.; project administration, Y.L. The author has read and agreed to the published version of the manuscript.}

\funding{This research received no external funding.}

\institutionalreview{Not applicable. The study uses machinery signals and involves no human or animal participants.}

\informedconsent{Not applicable.}

\dataavailability{The raw MCC5-THU Motor data are publicly available through Mendeley Data, Version 1, at \url{https://doi.org/10.17632/6s3dggj9mw.1} and are not redistributed. Frozen source-recording split maps, exact input schemas, result tables, and reproduction scripts are included in the accompanying software supplement. A permanent public archive will be added to the accepted version.}

\acknowledgments{During the preparation of this manuscript, the author used OpenAI ChatGPT and OpenAI Codex (service versions accessed in July 2026) for code review, language editing, and document organization. The author reviewed and edited all outputs and takes full responsibility for the content of this publication. All scientific analyses were executed locally, and all numerical claims were checked against saved artifacts.}
...
\bibitem{You2024SoundVibrationPhysics} Keshun, You; Puzhou, Wang; Peng, Huang; Yingkui, Gu. A sound-vibration physical-information fusion constraint-guided deep learning method for rolling bearing fault diagnosis. Reliability Engineering \& System Safety. 2025; 253: 110556. doi:\url{10.1016/j.ress.2024.110556}.
