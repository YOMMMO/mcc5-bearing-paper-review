...
\begin{landscape}
\begin{figure}[p]
\centering\includegraphics[width=0.98\linewidth]{figures/figure_s3_per_recording_probability.pdf}
...
The verified runtime used Python 3.12.13, PyTorch 2.11.0+cu128, and an NVIDIA GeForce RTX 5080. Automated tests cover train-only sequence and scalar standardization, constant channels, missing scalar values, serialization of fitted statistics, source-recording separation across all data roles, and the evidence-status labels applied to exploratory configurations. The software supplement includes frozen split maps, a de-identified window-to-recording index, exact input schemas, result tables, and reproduction commands; raw MCC5 signals are not redistributed.
